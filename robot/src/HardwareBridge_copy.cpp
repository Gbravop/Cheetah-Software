...
#include "config.h" /IMU Configuration
#include "rt_imu.h" /IMU Header
...
void MiniCheetahHardwareBridge::runSpi() {
  ...
  _spiLcm.publish("spi_command",gyro cmd);
}
...
void Cheetah3HardwareBridge::publishEcatLCM() {
  for(int leg = 0; leg < 4; leg++) {
    ...
    ecatCmdLcm.q_des_hip[_robotRunnerleg] = _tiBoardCommand[leg].q_des[1];
  ...
  for(int leg = 0; leg < 4; leg++) {
    ...
    ecatDataLcm.dx[leg] =_robotRunner _tiBoardData[leg].velocity[0];
...
// /*
void Cheetah3HardwareBridge::runKVH() {
  /**** init_serial ****/
    init_serial();
  /**********************/
    while(true) {
...
#ifdef USE_KVH
  _vectorNavData.accelerometer << imu_d[3], imu_d[4], imu_d[5];
  _vectorNavData.gyro << imu_d[0], imu_d[1], imu_d[2];
#endif
  }
}
// */
...
/*
void Cheetah3::handelIMU(){
  while(true){
  acc = getIMUData();
  vel = int(acc);
  pos = int(vel);_robotRunner

  _vectorNavData.accelerometer = acc;
  _vectorNavData.quat[0] = pos.quat[1];
  _vectorNavData.quat[1] = pos.quat[2];
  _vectorNavData.quat[2] = pos.quat[3];
  _vectorNavData.quat[3] = pos.quat[0];
  _vectorNavData.gyro = vel.gyro;
}
